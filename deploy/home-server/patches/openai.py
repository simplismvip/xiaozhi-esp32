...
THINKING_DISABLED_DOMAINS = {
    ...
    # MiniMax-M3 可关闭思考；M2.x 会忽略该参数，需配合下方 content 过滤
    "minimaxi.com": {"thinking": {"type": "disabled"}},
    "minimax.io": {"thinking": {"type": "disabled"}},
}
        ...
        try:
            for chunk in stream:
                if getattr(chunk, "choices", None):
                    ...
                    # function_call 路径原先未过滤 think，会导致 TTS 朗读思考内容
                    if content:
                        if "<think>" in content:
                            is_active = False
                            content = content.split("<think>")[0]
